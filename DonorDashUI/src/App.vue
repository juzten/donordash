    ...
    <v-app>
        ...
        <v-app-bar app dark clipped-left>
            ...
            <v-toolbar-title class="headline text-uppercase">
                <span class="blue--text"> <v-icon class="blue--text" style="font-size: 38px;">face</v-icon>Donor </span>
                <span class="font-weight-light red--text">Dash</span>
            </v-toolbar-title>
            <v-spacer />
            <v-flex xs12 md4>
...
export default {
    ...
    computed: {
        removedAnonymous() {
            return this.donations.filter(obj => !obj.anonymous)
        }
    },
    created() {
        this.$donations_api
            ...
            .then(async response => {
                response.data.forEach(element => {
                    ...
                    this.total_donations = parseFloat(this.total_donations) + parseFloat(element.donation_amount)
                })
            })
            .catch(error => {
